...
class Calculator(swing.JFrame):
    ...
    def __init__(self):
        ...
        self.contentPane.add(self.create_operator_panel(),
    ...
    def operator_pressed(self, event):
        self.handle_operator(event.source.text)
        
    def key_pressed(self, event):
        char = event.keyChar
        if char.isdigit():
            self.handle_digit(char)
        elif char in self.operators_labels:
    ...
    def compute_result(self, curr_expr):
        return eval(curr_expr)
    
    def handle_digit(self, digitChar):
        self.display.text += digitChar
    
    def handle_operator(self, opChar):
        curr_expr = self.display.text
        
        if curr_expr and curr_expr[-1].isdigit():
            if opChar == '=':
                res = self.compute_result(curr_expr)
                self.display.text = str(res)
                pass
            else:
                self.display.text += opChar
